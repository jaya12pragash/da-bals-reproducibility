"""Final confirmation on fresh robustness seeds 200--229."""
...
ROBUSTNESS_SEEDS = tuple(range(200, 230))
POLICIES = ("DA-BALS-NoMargin", "DA-BALS-FixedMargin",
            "DA-BALS-Adaptive", "DA-BALS-Stabilized")


def main():
    manifest = json.loads((ROOT / "results" / "frozen_parameters.json").read_text())
    frozen = manifest["selected_parameters"]
    rows = []
    total = len(ROBUSTNESS_SEEDS)*len(LOAD_POINTS)*len(TAU_LEVELS)*len(CV_LEVELS)*len(POLICIES)
    completed = 0
            ...
            for tau in TAU_LEVELS:
                for cv in CV_LEVELS:
                    for policy in POLICIES:
                        outcomes = []
                        simulate(seed, load, tau, cv, policy, requests, background, outcomes,
                                 tuning_params=frozen if policy=="DA-BALS-Stabilized" else None)
                        frame = pd.DataFrame(outcomes)
                        assert len(frame)==len(requests)
                        rows.append({"seed":seed,"load":load,"tau_ms":tau,"cv":cv,"policy":policy,
                            "requests":len(frame),"on_time_fraction_pct":100*frame.status.eq("Success").mean(),
                        ...
                        if completed%500==0 or completed==total:
                            print(f"completed {completed}/{total} robustness configurations",flush=True)
    output=ROOT / "results"; output.mkdir(exist_ok=True)
    runs=pd.DataFrame(rows); runs.to_csv(output/"robustness_run_metrics.csv",index=False)
    comparisons=[]
    for load in LOAD_POINTS:
        for tau in TAU_LEVELS:
            for cv in CV_LEVELS:
                ...
                for baseline in POLICIES[:-1]:
                    ...
                    mean=delta.mean(); half=stats.t.ppf(.975,29)*stats.sem(delta)
                    comparisons.append({"load":load,"tau_ms":tau,"cv":cv,"baseline":baseline,"pairs":30,
                        "mean_delta_pp":mean,"ci95_low_pp":mean-half,"ci95_high_pp":mean+half,
                        "p_value":stats.ttest_1samp(delta,0).pvalue})
    paired=pd.DataFrame(comparisons)
    paired["holm_p_value_global_162"]=holm_adjust(paired.p_value)
    paired.to_csv(output/"robustness_paired_comparisons.csv",index=False)
    ...
    with (output/"robustness_report.txt").open("w") as report:
    ...
    print((output/"robustness_report.txt").read_text())
